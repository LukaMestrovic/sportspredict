...
def _intent_task(request_path: Path, response_path: Path, request: dict) -> str:
    return (
        ...
        "Assess each contract catalogue-first for the downstream pricing path: "
        "first look for an exact pre-match market in "
        "`soccer_live_odds_market_catalog.pdf`; never treat a live/in-play market "
        "ID as pre-match direct evidence. If no exact catalogue contract exists, "
        "look next for exact web odds. If exact odds are unavailable, declare a "
        "semantically related live-odds proxy and combine it with the simulator, "
        "rather than presenting the proxy as direct. If only one of proxy or "
        "simulator is usable, disclose that one-source basis; use researched base "
        "rates as the final fallback. Keep the response limited to the unchanged "

"""Auditable final LLM pricing from match evidence.

This layer receives the deterministic evidence JSON for one match and returns
final SportPredict probabilities. It does not tilt anchors: every submitted
    ...
    *,
    ...
    for market in result.markets:
        ...
        pred = Prediction(
            ...
            n_books=len(direct),
            market_label="LLM final audited price",
            source="llm-pricing",
...
def _markdown_report(result, evidence: dict, evidence_path: Path | None, response: dict) -> str:
    ...
    for market in result.markets:
        ...
        if not audit:
            ...
            continue
        lines.append(f"- probability: {audit.get('probability_int')}%")
